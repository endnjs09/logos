# ADR 002: Plugin Boundary

## Status

Accepted.

## Decision

Logos will support plugins as external harness packs. The initial structure
includes `plugins/` and plugin manifest schemas, but runtime plugin loading is
deferred. Plugin runtime code, when introduced, should live under `packages/`.

## Rationale

Logos needs replaceable calibration assets: commands, roles, skills, hooks,
guards, prompts, workflows, benchmarks, schemas, and evaluators. Keeping these
as plugins allows new calibration strategies without modifying the core runtime.
